...
def set_infer_dir():
    ...
    """
    if not os.path.exists('content/outputs/inference'):
        os.makedirs('content/outputs/inference')
    num_infer_dirs_present = len(os.listdir('content/outputs/inference/'))
...
def set_training_dir(dir_name=None):
    ...
    And returns the directory path.
    """
    if not os.path.exists('content/outputs/training'):
        os.makedirs('content/outputs/training')
    if dir_name:
        new_dir_name = f"content/outputs/training/{dir_name}"
        os.makedirs(new_dir_name, exist_ok=True)
        return new_dir_name
    else:
        num_train_dirs_present = len(os.listdir('content/outputs/training/'))
        next_dir_num = num_train_dirs_present + 1
        new_dir_name = f"content/outputs/training/res_{next_dir_num}"
        os.makedirs(new_dir_name, exist_ok=True)
        return new_dir_name
